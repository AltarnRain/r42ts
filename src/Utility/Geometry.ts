 ...
 */
export function calculateAngle(start: GameLocation, end: GameLocation): number {
    let dx = Math.abs(start.left - end.left);
    let dy = Math.abs(start.top - end.top);

    // Bit of a hack here but if the dx = 0 or dy is zero you get the wrong angle.
    dy = dy === 0 ? 0.00000000001 : dy;
    dx = dx === 0 ? 0.00000000001 : dx;
