...
export default class FrameProvider {
     ...
     */
    constructor(frames: string[][][], startFrameIndex: number) {
        this.frames = frames;
    ...
    public getFrame(): string[][] {
        const returnValue = this.frames[this.currentFrameIndex];
    ...
    private setNextFrameIndex(): void {
        if ((this.currentFrameIndex + 1 > this.frames.length - 1) || this.currentFrameIndex === 0 ) {
            this.add *= -1;
        }
